    ...
    for payload in parser.parse_all(wasm) {
        ...
        match payload {
            ...
            wp::Payload::ImportSection(imports) => {
                maybe_add_imports(env, &mut new_type_section, &mut new_import_section);
                transform_imports_section(imports, &mut new_import_section)?;
            }
            ...
            wp::Payload::ElementSection(reader) => {
                transform_elem_section(reader, &mut new_element_section)?;
            ...
            wp::Payload::CodeSectionEntry(reader) => {
                maybe_add_imports(env, &mut new_type_section, &mut new_import_section);
                transform_code_section(reader, &mut new_code_section, analysis, wasm)?;
            }
            ...
            wp::Payload::CustomSection(reader) if reader.name() == "name" => {
                ...
                transform_name_section(names, &mut new_name_section)?;
...
fn maybe_add_imports(env: &str, ts: &mut we::TypeSection, is: &mut we::ImportSection) {
    if is.is_empty() {
