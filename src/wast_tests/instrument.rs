        ...
        for payload in parser.parse_all(module) {
            ...
            match payload {
                        ...
                        let items = match elem.items {
                            ...
                            wp::ElementItems::Expressions(exprs) => {
                                expressions = exprs
                                    .into_iter()
                                    .map(|v| {
                                        v.map_err(Error::ParseElementExpression).and_then(constexpr)
                                    })
                                    .collect::<Result<Vec<_>, _>>()?;
                ...
                wp::Payload::CodeSectionEntry(reader) => {
                    ...
                    while !operators.eof() {
                        let (op, offset) =
                            operators.read_with_offset().map_err(Error::ParseOperator)?;
                ...
                wp::Payload::CustomSection(reader) if reader.name() == "name" => {
                    ...
                    for name in names {
                        ...
                        match name {
                            ...
                            wp::Name::Global(map) => {
                                new_name_section.globals(&namemap(map, false)?)
                            }
                            wp::Name::Element(map) => {
